...
% Multiconstellation creating matrix 'A' with both contellations, WLS (not
% sure if works properly)
%
clearvars;
close all;
clc;
...
fprintf('\nMean time difference as computed from %2.0f epochs: %G seconds\n', Nepoch, t_dif_err_mean );
...
if Nepoch > 1   % Plot evolutions for all epochs
    ...
    title(sprintf('Elevation angles (%s, %s)', const));
...
%     saveas(fig, filename);
    ...
    fig = figure('DefaultAxesFontSize', 12); plot(TOW, t_dif_err, 'LineWidth', linewidth);
    xlabel('Time of the Week (s)');
    ylabel('Error in time (s)');
...
%     saveas(fig, filename);
    %
    % -- # of satellites
    fig = figure('DefaultAxesFontSize', 12); plot(TOW, sum(Nsat, 2), 'LineWidth', linewidth);
    hold on; plot(TOW, sum(Nsat, 2) - mask_sats, 'LineWidth', linewidth);
    legend('In view', 'Used');
    ylabel('Number of satellites');
    xlabel('Time of the Week (s)');
