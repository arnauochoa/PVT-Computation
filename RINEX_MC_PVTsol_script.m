...
% Multiconstellation averaging both constellations, LS
%
clearvars;
close all;
clc;
...
%     saveas(fig, filename);
    %
    % -- Error in time
    figure('DefaultAxesFontSize', 12); plot(TOW, t_err, 'LineWidth', linewidth);
    xlabel('Time of the Week (s)');
...
%     saveas(fig, filename);
    %
    % -- # satellites in view
    figure('DefaultAxesFontSize', 12); plot(TOW, Nsat, 'LineWidth', linewidth);
    legend(const);
    xlabel('Time of the Week (s)');
