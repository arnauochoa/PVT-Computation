...
%         end
        ...
        if (acq_info.flags.constellations.GPS && ~isempty(acq_info.SV_list.SVlist_GPS) || acq_info.flags.constellations.Galileo && ~isempty(acq_info.SV_list.SVlist_Galileo)) || multiCflag && ~isempty(acq_info.SV_list.SVlist_GPS) && ~isempty(acq_info.SV_list.SVlist_Galileo)
            ...
            if isempty(H)
                fprintf('Not enough satellites. Needed: %G\n', ((3 + acq_info.flags.constellations.GPS + acq_info.flags.constellations.Galileo)));
            else
...
%% Averaging
    if ~isempty(results) && ~isempty(PVTs)
